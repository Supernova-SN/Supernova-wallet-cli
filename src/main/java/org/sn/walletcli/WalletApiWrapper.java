...
public class WalletApiWrapper {
  ...
  //特殊函数，专门生成交易数据，用于离线签名
  public byte[] sendCoin2(String fromAddress, String toAddress, long amount)
	      throws CipherException, IOException, CancelException {
	    if (wallet == null || !wallet.isLoginState()) {
	      logger.warn("Warning: SendCoin failed,  Please login first !!");
	      return null;
	    ...
	    return wallet.sendCoin2(from, to, amount);
	  }
  ...
  public boolean assetIssue(String name, long totalSupply, int snNum, int icoNum, int precision,
