...
//转入交易记录
public class TransInPanel extends JPanel{
	private static final Logger logger = LoggerFactory.getLogger("TransInPanel");
	...
	JTable table;
	String[] title = {"交易","转入地址","金额","时间"};
	
	
	public TransInPanel(JFrame parent) {
...
//		groupbox.setPreferredSize(new Dimension(780, 100));
        ...
        txid.setCellEditor(new MyCellEditor());
        TableColumn address = table.getColumnModel().getColumn(1);
        address.setCellEditor(new MyCellEditor());
...
//				System.out.println("Easy jbQuerry Thread start...");
				...
				UI.cmd =3;
				WalletThread transferThread = new WalletThread(3, UI.address, 0, 1000);//新生成一个处理事务线程
