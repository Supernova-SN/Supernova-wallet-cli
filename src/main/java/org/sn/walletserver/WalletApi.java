...
import org.sn.api.GrpcAPI.EmptyMessage;
import org.sn.api.GrpcAPI.ExchangeList;
import org.sn.api.GrpcAPI.NodeList;
...
import org.sn.api.GrpcAPI.TransactionSignWeight.Result.response_code;
...
import org.sn.protos.Contract.BuyStorageBytesContract;
import org.sn.protos.Contract.BuyStorageContract;
...
import org.sn.protos.Contract.SellStorageContract;
...
import org.sn.protos.Protocol.TransactionSign;
...
//  }
  ...
  private Transaction signTransaction(Transaction transaction)
    ...
    while (true) {
      ...
      byte[] passwd = org.sn.keystore.StringUtils.char2Byte(password);
      org.sn.keystore.StringUtils.clear(password);
      ...
      org.sn.keystore.StringUtils.clear(passwd);
  ...
  //�����ת�ˣ�����from��to��amount���ɽ������ݣ��Լ���ǩ��Ҳ���㲥���������������������ǩ����
  public byte[] sendCoin2(byte[] from, byte[] to, long amount)
	      throws CipherException, IOException, CancelException {
	    Contract.TransferContract contract = createTransferContract(to, from, amount);
	    if (rpcVersion == 2) {
	      TransactionExtention transactionExtention = rpcCli.createTransaction2(contract);
	      Transaction transaction = transactionExtention.getTransaction();
  ...
  public static boolean passwordValid(char[] password) {
    if (ArrayUtils.isEmpty(password)) {
      //JOptionPane.showMessageDialog(null, "����Ϊnull");
      throw new IllegalArgumentException("password is empty");
    }
    if (password.length < 6) {
      //JOptionPane.showMessageDialog(null, "���볤��Ӧ��С��6λ");
...
//  }
  ...
  public byte[] generateContractAddress(Transaction sn) {

    // get owner address
    // this address should be as same as the onweraddress in sn, DONNOT modify it
    byte[] ownerAddress = getAddress();

    // get tx hash
    byte[] txRawDataHash = Sha256Hash.of(sn.getRawData().toByteArray()).getBytes();
  ...
  public boolean updateSetting(byte[] contractAddress, long consumeUserResourcePercent)
    ...
    if (transactionExtention == null || !transactionExtention.getResult().getResult()) {
      System.out.println("RPC create sn failed!");
      if (transactionExtention != null) {
  ...
  public boolean triggerContract(byte[] contractAddress, long callValue, byte[] data, long feeLimit,
    ...
    if (transactionExtention == null || !transactionExtention.getResult().getResult()) {
      System.out.println("RPC create call sn failed!");
      System.out.println("Code = " + transactionExtention.getResult().getCode());
      System.out
          .println("Message = " + transactionExtention.getResult().getMessage().toStringUtf8());
      return false;
